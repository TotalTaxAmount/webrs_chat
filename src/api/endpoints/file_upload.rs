...
impl Method for FileUpload<'_> {
    ...
    'r: 's,
  {
    ...
    let mime_type =
      match mime_guess::from_ext(f.extension().unwrap_or(OsStr::new("")).to_str()?).first() {
        Some(t) => {
          trace!("{}", t.essence_str());
          "text/plain"
        }
        None => {
          warn!(
            "[Request {}] Could not get mime type for {:?}",
            req.get_id(),
            f.as_os_str().to_str()
          );
          "text/plain"
    ...
    let data = match File::open(Path::new(
      format!("files/{}", f.as_os_str().to_str()?).as_str(),
    )) {
      Ok(mut f) => {
